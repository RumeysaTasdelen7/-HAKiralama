...
class ReservationCretaeAPIView(CreateAPIView):
    ...
    def post (self, request, formal=None):
        ...
        iha_id = request.query_params.get("ihaId")

        if not user_id or not iha_id:
            return Response(
                {
                    "error":"userId and ihaId are required"
                },
...
class ReservationAvailabilityAPIView(APIView):
    ...
    def get(self, request, format=None):
        ...
        if not iha_id or not pick_up_date_time or not drop_off_date_time:
            return Response(
                {
                    "error": "ihaId, pickUpDateTime and dropOffTime are required"
                },
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if reservations.exists():
            return Response(
                {
                    "message": "İha is not availeble for this time period"
                },
                status=status.HTTP_400_BAD_REQUEST
            )
        else:
            return Response(
                {
                    "message": "İHA is availeble for this time period",
                    "success": True
                },
...
class ReservationUpdateView(UpdateAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        if not reserv_id or not iha_id:
            return Response(
                {
                    "error": "id and ihaId are required"
                },
                status=status.HTTP_400_BAD_REQUEST
            )
